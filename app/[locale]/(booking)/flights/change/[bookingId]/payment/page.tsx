...
import FlightChangePaymentPageClient from "./FlightChangePaymentPageClient";
...
export async function generateMetadata({
  ...
  return metadataForLocalizedRoute(locale, "/flights/change/payment", {
...
export default async function Page({
  ...
  const returnPath = safeInternalPath(
    localizedCustomerPath(loc, `/flights/change/${encodeURIComponent(bookingId)}/payment`),
    localizedCustomerPath(loc, "/profile/bookings"),
  );
  if (!userId) {
    redirect(`/${locale}/login?next=${encodeURIComponent(returnPath)}`);
  }
  return <FlightChangePaymentPageClient bookingId={bookingId} />;
}
